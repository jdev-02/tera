...
from ml_dsa_signer import SignedPayload, create_signer, _sha256
...
def verify_cot(cot_xml: str, trust_list: dict[str, bytes] | None = None) -> VerificationResult:
    ...
    try:
        ...
        timestamp    = float(wayfinder.findtext("timestamp", "0"))
        ...
        if not sig_hex or not key_id or not payload_hash or not payload_json:
            return VerificationResult(
                valid=False, key_id=key_id, algorithm=algorithm,
        ...
        canonical_bytes = payload_json.encode()
        ...
        # Step 3: Full signature verification over payload bytes
        ...
        if algorithm == "Ed25519-fallback":
            ...
            pk = load_pem_public_key(pub_key) if pub_key else signer._pk
            try:
        ...
        if ok:
            return VerificationResult(
                valid=True, key_id=key_id, algorithm=algorithm,
        ...
        return VerificationResult(
            valid=False, key_id="", algorithm="",
            reason=f"CoT XML parse error: {e}"
        )
...
def export_public_key_to_trust_list(
    ...
    path: str = "crypto/keys/trust_list.json"
):
    """Exports this device's public key to the trust list JSON file."""
    from pathlib import Path
    from ml_dsa_signer import MLDSASigner, FallbackSigner
    p = Path(path)
...
if __name__ == "__main__":
    ...
    import re
    tampered = re.sub(r'"lat": 37\.7955', '"lat": 99.999', signed_xml)
