# Tactical Edge Route Agent — Product Requirements Document (PRD)

> **Working codename:** TBD — pick at Sat 1145 kickoff.
> **Status:** v0.2 — pre-build, decisions consolidated from refinement batches 1-3 + AI-in-CI/CD content.
...
A natural-language tactical route agent. The user speaks or types intent; an on-device LLM interprets it as a structured query over local geospatial data; a local routing engine returns a route; the route is signed with post-quantum credentials; ATAK renders it.

```
USER PROMPT (voice or text)
        │  (Whisper-tiny on Jetson for voice)
        ▼
...
SIGNING LAYER (ML-DSA / Dilithium signs each CoT field)
        │
        ▼
ATAK RENDERER (CoT over loopback / mesh) ──► verified blue line on operator's screen
```

**Hero claim for the pitch:** *"From spoken intent to ATAK-rendered route in under 30 seconds, with WiFi physically off — and we'll prove every track is authentic with post-quantum cryptography while we're at it."*
...
**Scenario A — "Plot a route to the nearest freshwater source"**

1. Operator opens ATAK on Android EUD; agent app is paired to Jetson over USB or BLE.
...
5. ATAK draws the route + waypoint pin + estimated time. Agent surfaces a 1-line rationale: *"Selected creek 2.1km NE; route follows draw to avoid ridgeline at GR 12345 67890."*
...
```mermaid
flowchart TD
    User[Operator] -->|"voice or text"| ATAK[ATAK on Android EUD or WinTAK]
    ...
    Agent --> STT["Whisper-tiny (voice path)"]
    ...
    ATAK -->|"verified blue line + pin"| User
...
| Voice      | STT                    | **Whisper-tiny** (39M params), CUDA on Jetson                                                              | Push-to-talk button; no always-on listening. P1 owns.                                                                   |
...
| Whisper-tiny WER on operator vocabulary   | < 10% on 20-prompt eval set          | P1 owns eval                                                                         |
...
| ----------------------------------- | ----------------------------------- | ------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **P1 — Navy Cyber Warfare Officer** | CS + AI, ontology focus             | `agent/`, `ontology/`, `eval/`, `voice/`               | LLM orchestration, prompt templates, JSON tool schema, NL→tag ontology, **Whisper-tiny voice path**, eval harness (20-prompt set with golden routes)                                                    |
| **P2 — Indonesian Navy**            | Cybersecurity                       | `security/`, `crypto/`, `infra/`, `.github/workflows/` | Threat model doc, **ML-DSA signer + verifier**, Jetson hardening script, egress firewall, `tcpdump` demo capture, CI security scans, SBOM, CODEOWNERS enforcement                                       |
| **P3 — USMC SIGINT Officer**        | Robotics, brought hardware          | `hardware/`, `deploy/`, `models/`, `mesh/` (stretch)   | Jetson bring-up, model quantize + benchmark, runtime selection (`ollama` vs `llama.cpp`), display rig, battery setup, **mesh substrate (phone + laptop + Nano)** for stretch, demo-day device wrangling |
| **P4 — USMC Combat Engineer**       | CS student, Mountain Warfare School | `atak/`, `routing/`, `data/`, `figma/`                 | ATAK CoT bridge (Android + WinTAK), Valhalla setup + custom cost model, OSM/DEM data pipeline for **both AOs**, scenario authoring, Figma user-journey mockup, SME route quality scoring                |
...
- ✅ Voice input is in MVP (Whisper-tiny on Jetson, push-to-talk).
...
- **OSM extract size** (county-level vs state-level for SF; the smaller, the safer for Jetson RAM).
- `**ollama` vs `llama.cpp`** for Gemma runtime — pick after P3's first benchmark Sat 1400.
